class CalendarCard extends HTMLElement {
  ...
  async getAllEvents(entities) {
    if(!this.lastUpdate || moment().diff(this.lastUpdate, 'minutes') > 15) {
      ...
      const end = moment().startOf('day').add(this.config.numberOfDays, 'days').format("YYYY-MM-DDTHH:mm:ssZ");

      let urls = entities.map(entity => `calendars/${entity}?start=${start}&end=${end}`);
